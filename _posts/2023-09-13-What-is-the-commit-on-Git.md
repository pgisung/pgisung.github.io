...
- <span id="head란-무엇일까"></span>**HEAD란 무엇일까?**
...
- <span id="snapshot-기반-접근-방식"></span>**Snapshot 기반 접근 방식**
...
- 실제로 실행해 보면 <span style="color: #8D4801">**바로 commit이 완료되는 것이 아니라 vim 에디터(또는 core.editor로 설정한 다른 에디터)가 출력**</span>되는 것을 확인할 수 있다. commit 메시지를 작성해야 하기 때문이다. 아래에서 간단한 사용법을 알아보자. ([Git 환경설정 中 core 에디터]({{ site.baseurl }}/github/2023/08/26/Getting-Started-with-Git/#git-config-core-editor "Navigate to Setting environment for Git"))
...
- <span id="commit-메시지"></span>**Commit 메시지**
  - 각각의 commit에 대한 정보를 입력할 수 있다.
  - 만약 commit.template을 작성해 뒀다면 vim 화면에 함께 출력된다. ([Git 환경설정 中 commit 템플릿]({{ site.baseurl }}/github/2023/08/26/Getting-Started-with-Git/#git-config-commit-template "Navigate to Setting environment for Git"))
...
- 가능하면 추천하고 싶진 않지만, 만약 필요하다면 commit 메시지가 없는 **빈 메시지의 커밋을 생성**하는 방법도 있다.
```bash
...
- <span id="commit-메시지-수정"></span>**Commit 메시지 수정**
```bash
git commit --amend
```
...
- <span id="commit-id"></span>**Commit ID**
...
#### 마무리하며...
이번 포스트에서는 Git의 핵심 요소인 Commit에 관하여 자세히 알아봤다. 앞으로 우리가 다룰 모든 명령어는 이 commit이라는 재료를 통해 실행되기 때문에 백번 강조해도 모자라지 않을 만큼 중요한 개념이다. 사실 어렵게 생각할 필요도 없다. 포스트를 시작하며 언급했던 것처럼 commit은 포도알 하나하나이고 이들을 모은 포도송이가 하나의 기능이 되는 것이다. 다음 포스트 때는 기록해 둔 commit들을 원격 저장소로 전송하고 다시 로컬 저장소에 내려받아 보자.